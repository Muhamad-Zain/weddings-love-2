    ...
    return(
        <section className='relative'>
          <div className={`${style.bg} `}>
            <div className="relative w-full h-screen sm:h-screen overflow-hidden">
              <Slider {...settings}>
                {images.map((img, i) => (
                  <div key={i}>
                    <img
            ...
            <AnimateSee>
              <div className='absolute text-white z-30 bottom-16  w-full text-center   '>
                <h3 style={{textShadow: "1px 1px #5e86a8"}} className='text-opacity-95 text-5xl GVibes tracking-wider font-bold  border-b inline ' >{data?.name?.mens} & {data?.name?.grils}</h3>
